...
class ReplyRepositoryPostgres extends ReplyRepository {
  ...
  async verifyReplyOwner(replyId, userId) {
    ...
    if (!result.rowCount) {
      throw new AuthorizationError('tidak bisa menghapus balasan orang lain.');
    }
  }
  ...
  async getReplies(threadId) {
    const query = {
      text: `
        SELECT replies.id, replies.comment_id, users.username, replies.created_at AS date, 
        replies.content, replies.deleted_at, replies.is_delete 
        FROM replies 
